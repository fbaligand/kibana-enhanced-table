...
export interface DocumentTableVisRenderValue {
    visType: string;
    visData: object;
    visConfig: object;
    params?: object;
...
export type DocumentTableExpressionFunctionDefinition = ExpressionFunctionDefinition<
  ...
  any,
  Arguments,
  Promise<Render<DocumentTableVisRenderValue>>
>;

export const documentTableExpressionFunction = (): DocumentTableExpressionFunctionDefinition => ({
  ...
  async fn(input, args, { inspectorAdapters }) {
    ...
    const aggs = indexPattern
      ...
      : undefined;
    const visType = getVisualization().get(DOC_TABLE_VIS_NAME)
    ...
    input = await documentTableResponseHandler(input);
    

    return {
      type: 'render',
      as: DOC_TABLE_VIS_NAME,
      value: {
        visData: input,
