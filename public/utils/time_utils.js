function incrementDateUnit(date, incrementCount, unit) {
  if (unit === 'y') {
...
function roundDate(roundUnit, roundUp, date, dayOfWeekNumber) {
  if (roundUp) {
...
function parseDathMathExpression(dateMathExpression, roundUp, nowReference, dayOfWeekNumber) {
 ...
 */
export function computeDurationStructureBrokenDownByTimeUnit(durationInMillis) {
 ...
 */
export function computeTimeRange(timeRange, dayOfWeek) {
  ...
  try {
    ...
    const from = parseDate(timeRange.from, false, nowReference, dayOfWeekNumber);
    const to = parseDate(timeRange.to, true, nowReference, dayOfWeekNumber);
