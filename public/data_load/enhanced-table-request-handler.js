...
export async function enhancedTableRequestHandler ({
  ...
  if (visParams.fieldColumns !== undefined) {
    ...
    searchSourceFields.docvalue_fields = visParams.fieldColumns.filter(fieldColumn => !fieldColumn.field.name.startsWith('_') && fieldColumn.field.aggregatable).map(fieldColumn => fieldColumn.field.name);
    const scriptFields = {};
