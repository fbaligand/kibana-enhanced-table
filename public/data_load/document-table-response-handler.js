...
function createColumn(fieldColumn, index, aggConfigs) {
  ...
  const aggConfigOptions = {
    ...
    schema: 'bucket',
    params: {
      field: aggConfigs.indexPattern.fields.byName[fieldColumn.field.name]
    }
  };
...
export function documentTableResponseHandler(table) {
  return { tables: [ createTable(table) ], totalHits: table.totalHits, newResponse: true };
}
